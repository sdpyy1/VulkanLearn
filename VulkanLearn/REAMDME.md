...
5. 创建管线（整个渲染流程中的配置设置）
    ...
    - 设置RenderPass (渲染过程中如何使用帧缓冲区附件（如颜色缓冲、深度缓冲）),比如新的一帧开始时清除颜色缓冲，另外还有子通道也在这里
6. 创建帧缓冲区
	- 每个交换链的缓冲区都需要一个对应的帧缓冲区（vk的FrameBuffer与OpenGL概念不一致啊，OpenGL的双缓冲都是同一个FBO下）
7. 创建命令缓冲区
	- 创建命令池
	- 分配命令缓冲区
...
# 设置顶点数据
1. 硬编码到shader
2. 创建顶点缓冲区，创建顶点描述信息。需要手动申请内存，绑定缓冲区，映射内存，复制数据
3. 2的顶点缓冲区CPU可访问，但是GPU读取慢。  GPU有一类缓存CPU无法访问，但是GPU读取快。  解决方案：创建两个缓冲区，一个CPU可访问的作为中转站，一个GPU可访问的作为最终顶点缓冲区。每次更新数据时，先把数据传到CPU可访问的缓冲区，然后通过命令拷贝到GPU可访问的缓冲区
4. 另外 也可以设置IndexBuffer表示顶点索引顺序

# 传递Uniform变量
1. 创建Uniform Buffer,创建描述信息

# 纹理
1. 加载图片
2. 临时缓冲区读取图片数据
3. 传递数据到纹理图片，并且需要修改布局
4. 为纹理图片创建ImageView
5. 采样器设置，采样器用来设置过滤、边缘处理、各向异性过滤等
